...
When `force_delete` is set to `true` (the default), running `terraform destroy` will automatically delete all dependent resources (prompts, prompt partials, configs, guardrails, virtual keys) in the workspace before deleting the workspace itself. Set to `false` if you want the Portkey API to reject workspace deletion when dependent resources exist.
...
- `force_delete` (Boolean) If true, automatically deletes all dependent resources (prompts, prompt partials, configs, guardrails, virtual keys) before deleting the workspace. This is a Terraform-only convenience flag; the Portkey API does not support cascade deletion natively. Default: `true`.
