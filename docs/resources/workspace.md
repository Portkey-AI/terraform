...
```terraform
resource "portkey_workspace" "ephemeral" {
  name                       = "Ephemeral"
  description                = "Short-lived workspace for testing"
  delete_dependent_resources = true
}
```

When `delete_dependent_resources` is set to `true`, running `terraform destroy` will automatically delete all dependent resources (prompts, prompt partials, configs, guardrails, virtual keys) in the workspace before deleting the workspace itself. Without this flag, the Portkey API rejects workspace deletion if any dependent resources exist.
...
- `delete_dependent_resources` (Boolean) If true, automatically deletes all dependent resources (prompts, prompt partials, configs, guardrails, virtual keys) before deleting the workspace. This is a Terraform-only convenience flag; the Portkey API does not support cascade deletion natively. Default: `false`.
