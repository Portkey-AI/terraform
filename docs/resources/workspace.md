...
When `force_delete` is set to `true` (the default), running `terraform destroy` will automatically delete all dependent resources (prompts, prompt partials, configs, guardrails, virtual keys) in the workspace before deleting the workspace itself — including resources created outside Terraform. Set to `false` if you want the Portkey API to reject workspace deletion when dependent resources exist.
