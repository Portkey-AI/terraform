...
By default (`force_delete = false`), the Portkey API rejects workspace deletion with `409 AB07` while dependent resources still exist. Resources that Terraform manages are destroyed first by its own dependency graph, so this only bites when the workspace also holds resources created outside Terraform.

Set `force_delete = true` to have the provider delete all dependent resources (prompts, prompt partials, configs, guardrails, virtual keys) before deleting the workspace. **This includes resources created outside Terraform** — anything added through the Portkey UI or another tool in that workspace is destroyed too. Enable it only when the workspace's entire contents are disposable.
